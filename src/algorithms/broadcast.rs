...
impl Broadcast {
    pub fn new() -> Self {
        Self { node_id: "n0".to_string(), topology: HashMap::new(), values: vec![] }
    }
...
impl NodeHandler for Broadcast {
    ...
    async fn handle(&mut self, msg: &str, next_msg_id: u64) -> Result<String, errors::ErrorMsg> {
        ...
        let mut values = match &msg_in.body {
            broadcast::BroadcastMsgRequestBody::Topology(msg) => {
                self.handle_topology(&msg)
            },
            broadcast::BroadcastMsgRequestBody::Broadcast(msg) => {
                self.handle_broadcast(&msg)
            },
            broadcast::BroadcastMsgRequestBody::Read(msg) => {
